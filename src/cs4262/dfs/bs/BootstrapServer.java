...
public class BootstrapServer {
    ...
    public static void main(String args[]) {
        ...
        try {
            DFSProperties dFSProperties = DFSProperties.getInstance();
            sock = new DatagramSocket(Integer.parseInt(
                    dFSProperties.getProperty("bs.port", "5000")));

            echo("Bootstrap Server created at " 
                    +dFSProperties.getProperty("bs.port", "5000")
            ...
            while (true) {
                ...
                if (command.equals("REG")) {
                    ...
                    if (nodes.size() == 0) {
                        ...
                        if (isOkay) {
                            if (nodes.size() == 1) {
                                reply += "1 " + nodes.get(0).getIp() + " " 
                                        + nodes.get(0).getPort();
                            } else if (nodes.size() == 2) {
                                reply += "2 " + nodes.get(0).getIp() + " " 
                                        + nodes.get(0).getPort() + " " 
                                        + nodes.get(1).getIp() + " " 
                                        + nodes.get(1).getPort();
                            } else {
                                ...
                                reply += "2 " + nodes.get(random_1).getIp() 
                                        ...
                                        + " " + nodes.get(random_2).getPort();
                            }
                            nodes.add(new Neighbour(ip, port, username));
                        }
                    }
                    DatagramPacket dpReply = new DatagramPacket(
                            reply.getBytes(), reply.getBytes().length,
